...
export class Globals extends Service<Globals> {
  ...
  public register({ before = [], after = [] }: RegisterGlobalsOptions): void {
    ...
    const mount = ({ component: Component, id }: GlobalConfig): HTMLElement | null => {
      ...
      // Removes any statically pre-rendered element with this id (from the SEO/SSR build)
      // before mounting the live one, mirroring ViewRenderManager.clearStaleMountContent's
      // "wipe stale content, mount live" convention — otherwise both stay in the DOM.
