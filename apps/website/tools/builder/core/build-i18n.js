...
const destDir = './dist-build/src/locales';
const cleanupDir = './dist-build/src/app';

function findLocaleFiles() {
  const allFiles = getFilesRecursive(sourceRoot, '.json');
  const localeFileMap = {};
...
async function removeJsonFiles(directory) {
  ...
  for (const file of jsonFiles) {
    if (file.includes(`${path.sep}locales${path.sep}`)) {
      try {
        ...
        print.info(`Deleted locale JSON file: ${file}`);
      } catch (err) {
  ...
  print.info(`✅ JSON files cleanup complete.`);
}
